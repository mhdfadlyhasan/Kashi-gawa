...
export async function tokenizeText(text: string): Promise<Token[]> {
  ...
  const merged: Token[] = [];
  const auxSuffixes = new Set(['て', 'で', 'た', 'だ', 'たら', 'ても', 'でも', 'ては', 'では', 'たり', 'たい', 'ない']);
  ...
  for (let i = 0; i < mapped.length; i++) {
    ...
    // Backward merge: ない attaches to the previous token even if it was mis-tagged.
    // We block pure particles (except て/で which form ~ていない / ~でない).
    if (
      current.surface_form === 'ない' &&
